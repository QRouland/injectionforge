...
use frida::{
    DeviceManager, DeviceType, Frida, Message, MessageLogLevel, ScriptHandler, ScriptOption,
    ScriptRuntime, SpawnOptions,
};
...
pub fn attach_with(frida_code: &str, mode: AttachMode) {
    ...
    if let Ok(mut device) = device_manager.get_device_by_type(DeviceType::Local) {
        ...
        let pid = match mode {
            ...
            AttachMode::Name(ref name) => device
                .enumerate_processes()
                .iter()
        ...
        let mut script_option = ScriptOption::new().set_runtime(ScriptRuntime::QJS);
